...
func (m *model) resizePanel(panel string, delta int) {
	...
	// Update panel sizes with new ratios
	m.updatePanelSizes()
}
...
func (m model) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case tea.KeyMsg:
		...
		// Check resize keys first to avoid conflicts
		...
		case m.config.Keybindings.Resize.ShrinkEditorLeft:
			...
			m.statusMessage = fmt.Sprintf("Layout: %d%% | %d%% | %d%%", m.connectionRatio, m.editorRatio, m.resultsRatio)
			return m, nil
		...
		if m.layoutPresetMode {
			...
			case m.config.Keybindings.Layout.PresetDefault:
				m.setLayoutPreset(1)
				m.statusMessage = fmt.Sprintf("Layout: %d%% | %d%% | %d%% (Default)", m.connectionRatio, m.editorRatio, m.resultsRatio)
				m.layoutPresetMode = false
				return m, nil
			case m.config.Keybindings.Layout.PresetEditor:
				m.setLayoutPreset(2)
				m.statusMessage = fmt.Sprintf("Layout: %d%% | %d%% | %d%% (Editor Focus)", m.connectionRatio, m.editorRatio, m.resultsRatio)
				m.layoutPresetMode = false
				return m, nil
			case m.config.Keybindings.Layout.PresetResults:
				m.setLayoutPreset(3)
				m.statusMessage = fmt.Sprintf("Layout: %d%% | %d%% | %d%% (Results Focus)", m.connectionRatio, m.editorRatio, m.resultsRatio)
				m.layoutPresetMode = false
				return m, nil
			case m.config.Keybindings.Layout.PresetBalanced:
				m.setLayoutPreset(4)
				m.statusMessage = fmt.Sprintf("Layout: %d%% | %d%% | %d%% (Balanced)", m.connectionRatio, m.editorRatio, m.resultsRatio)
				m.layoutPresetMode = false
				return m, nil
			case "esc":
				m.layoutPresetMode = false
		...
		switch key {
		case m.config.Keybindings.Global.Quit, "ctrl+c":
			// Save connections before quitting
			m.saveConnections()
			return m, tea.Quit
		...
		case m.config.Keybindings.Global.OpenNeovim:
			// Open in Neovim with schema context injection
			if editor.IsNvimAvailable() {
				query := m.editorPanel.GetQuery()
				// Get active connection for schema context
				activeConn, err := m.connManager.GetActive()
				var conn db.Connection
				if err == nil {
					conn = activeConn
				}
				// Inject schema if enabled in config (default: true)
				injectSchema := true
				if m.config.AI != nil {
					injectSchema = m.config.AI.InjectInNeovim
				}
